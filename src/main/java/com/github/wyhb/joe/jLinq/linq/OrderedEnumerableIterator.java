...
class OrderedEnumerableIterator<TElement, TKey> implements IEnumerable<TElement> {
	...
	OrderedEnumerableIterator(IEnumerable<TElement> source, Function<TElement, TKey> keySelector,
			Comparator<TKey> comparer, boolean descending) {
		this.source = source;
		this.parent = null;
	...
	public OrderedEnumerableIterator createOrderedEnumerable(Function keySelector, Comparator comparer,
			boolean descending) {
	...
	class EnumerableSorter<TElement, TKey> extends IEnumerableSorter<TElement> {
		...
		EnumerableSorter(Function<TElement, TKey> keySelector, Comparator<TKey> comparer, boolean descending,
				IEnumerableSorter<TElement> next) {
			this.keySelector = keySelector;
			this.comparer = comparer;
			this.descending = descending;
			this.next = next;
		...
		void computeKeys(List<TElement> elements, int count) {
			keys = new ArrayList<>();
			for (int i = 0; i < count; i++)
				keys.add(keySelector.apply(elements.get(i)));
			if (next != null)
